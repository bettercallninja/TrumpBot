...
from src.database.db_manager import initialize_pool, DBManager
...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler(logs_dir / 'bot_errors.log', encoding='utf-8', level=logging.ERROR)
    ]
)
...
class BotApplication:
    ...
    def create_bot_instance(self) -> bool:
        ...
        try:
            ...
            # Add bot information logging
            if hasattr(self.bot, 'get_me'):
                try:
    ...
    def configure_error_handling(self) -> bool:
        ...
        try:
            ...
            # Configure bot-wide error handler
            @self.bot.middleware_handler(update_types=['message', 'callback_query'])
            def error_middleware(bot_instance, update):
                """Middleware for tracking user language and activity"""
                try:
                    user_id = None
                    ...
                    if hasattr(update, 'from_user') and update.from_user:
                        ...
                        # Detect user language from locale or previous interactions
                        language = detect_user_language(user_id) or self.default_language
                    
                    # Record metrics
                    self.metrics.record_message(language)
                    
                    # Determine feature being used
                    feature = 'unknown'
                    if hasattr(update, 'text') and update.text:
                        if update.text.startswith('/'):
                            feature = update.text.split()[0]
                        else:
                            feature = 'text_message'
            ...
            def handle_exceptions(exception):
                """Global exception handler for bot errors"""
                self.metrics.record_error()
                
                error_id = str(uuid.uuid4())[:8]
                error_msg = str(exception)
                
                logger.error(f"Bot exception [{error_id}]: {type(exception).__name__}: {error_msg}")
                logger.error(f"خطای ربات [{error_id}]: {type(exception).__name__}: {error_msg}")
    ...
    def run(self):
        ...
        try:
            ...
            # Start polling with infinity polling for stability
...
def perform_health_check() -> bool:
    ...
    try:
        ...
        try:
            # This is a simplified check - in production, would test actual connection
            config = BotConfig()
...
def validate_configuration() -> bool:
    ...
    try:
        ...
        missing_settings = []
        for attr, name in required_settings.items():
